...
// 1. Trust Proxy: Required on Render to accurately read client IPs for rate limiting[span_0](start_span)[span_0](end_span)
app.set('trust proxy', 1);

// 2. Security Headers: Protects against common web vulnerabilities[span_1](start_span)[span_1](end_span)
app.use(helmet());

// 3. CORS Setup: Vital for GitHub Pages to communicate with Render[span_2](start_span)[span_2](end_span)
// Replace the default value with your actual GitHub Pages URL in production via Render Environment Variables[span_3](start_span)[span_3](end_span)
...
// 4. Request Logging: Standardized Apache combined log output for Render dashboards[span_4](start_span)[span_4](end_span)
app.use(morgan('combined'));

// 5. Payload Limits: Prevent Denial of Service (DoS) via massive JSON payloads[span_5](start_span)[span_5](end_span)
app.use(express.json({ limit: '500kb' }));

// 6. Rate Limiting: Maximum 25 chat requests per minute per IP address[span_6](start_span)[span_6](end_span)
...
// Fail fast if the environment is completely misconfigured[span_7](start_span)[span_7](end_span)
if (apiKeys.length === 0) {
  console.error('CRITICAL ERROR: No GEMINI_API_KEY environment variables found.');
  process.exit(1); // Stop the server immediately if there is no API key[span_8](start_span)[span_8](end_span)
}
...
// Core System Instruction matching MindGuard-AI branding[span_9](start_span)[span_9](end_span)
...
4. Wellbeing Assessment: Help users reflect on daily patterns (mood, stress, sleep) and suggest manageable, actionable coping strategies[span_10](start_span)[span_10](end_span).
5. Safety Protocol: You are an AI early-support tool, NOT a diagnostic medical doctor or therapist[span_11](start_span)[span_11](end_span).
   - If a user expresses severe distress, self-harm, or suicidal ideation, respond with immediate compassionate support alongside official helpline details (e.g., 988 Suicide & Crisis Lifeline or local emergency resources)[span_12](start_span)[span_12](end_span).
6. Tone: Warm, conversational, highly supportive, clear, and grounded[span_13](start_span)[span_13](end_span). Act as a non-judgmental confidant.
`;
...
// 8. Health Check Route: Render uses this to ensure your server is alive[span_14](start_span)[span_14](end_span)
...
app.post('/api/chat', chatLimiter, async (req, res) => {
  try {
    ...
    // SLIDING WINDOW: Pass only the last 15 messages to Gemini[span_15](start_span)[span_15](end_span)
    // Prevents payload bloating, lowers token usage, and protects speed[span_16](start_span)[span_16](end_span)
    ...
    // Check for specific Google AI quota errors[span_17](start_span)[span_17](end_span)
...
// 10. Global Error Handler for unhandled middleware issues[span_18](start_span)[span_18](end_span)
